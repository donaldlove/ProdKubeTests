...
#get list of pods
curl --insecure https://35.189.126.19:6443/api/v1/pods -H "Authorization: Bearer $KUBE_TOKEN"  && \
#peek dashboard
echo "now peek Dashboard: \n"
curl --insecure https://35.189.126.19:6443/api/v1/namespaces/kube-system/pods/kubernetes-dashboard-57df4db6b-2fwtv -H "Authorization: Bearer $KUBE_TOKEN"
#dashboard port is at 31958, test it
if curl --insecure https://35.189.126.19:31958/#!/login; then echo "works"
else
	echo "not working"
fi